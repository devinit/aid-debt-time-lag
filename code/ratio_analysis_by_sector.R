...
pre_subset_sums = crs[,.(USD_Disbursement_Defl_original=sum(USD_Disbursement_Defl, na.rm=T)), by=.(SectorName)]
crs$CommitmentYear = as.numeric(substr(crs$CommitmentDate, 1, 4))
crs = subset(crs, !is.na(CommitmentYear))
post_subset_sums = crs[,.(USD_Disbursement_Defl_subset=sum(USD_Disbursement_Defl, na.rm=T)), by=.(SectorName)]
subset_analysis_df = merge(pre_subset_sums, post_subset_sums, by="SectorName", all=T)
...
fwrite(subset_analysis_df, "output/sector_without_commitment.csv")
