...
pre_subset_sums = crs[,.(USD_Disbursement_Defl_original=sum(USD_Disbursement_Defl, na.rm=T)), by=.(DonorName)]
crs$CommitmentYear = as.numeric(substr(crs$CommitmentDate, 1, 4))
crs = subset(crs, !is.na(CommitmentYear))
post_subset_sums = crs[,.(USD_Disbursement_Defl_subset=sum(USD_Disbursement_Defl, na.rm=T)), by=.(DonorName)]
subset_analysis_df = merge(pre_subset_sums, post_subset_sums, by="DonorName", all=T)
...
fwrite(subset_analysis_df, "output/donor_without_commitment.csv")
...
for(year in analysis_years){
  ...
  total_disbursed_by_commitments_this_year = all_committed_this_year[,.(USD_Disbursement_Defl=sum(USD_Disbursement_Defl, na.rm=T)), by=.(DonorCode)]
...
donor_codelist = unique(crs[,c("DonorCode", "DonorName")])
...
aid_debt_df = merge(aid_debt_df, donor_codelist)

aid_debt_agg = aid_debt_df[,.(USD_Disbursement_Defl=sum(USD_Disbursement_Defl), USD_Commitment_Defl=sum(USD_Commitment_Defl)), by=.(DonorCode)]
aid_debt_agg$aid_debt_ratio = aid_debt_agg$USD_Disbursement_Defl / aid_debt_agg$USD_Commitment_Defl

aid_debt_agg = merge(aid_debt_agg, donor_codelist)
fwrite(aid_debt_agg, "output/aid_debt_by_donor.csv")
